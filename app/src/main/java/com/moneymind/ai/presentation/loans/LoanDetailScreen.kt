...
fun LoanDetailScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(loan?.personName ?: "Loan") },
                navigationIcon = {
        ...
        Column(
            ...
            Card {
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Text(
                        text = CurrencyFormatter.format(state.outstanding),
                        style = MaterialTheme.typography.displayLarge
                    )
                    Text(
                        text = "${CurrencyFormatter.format(state.totalRepaid)} repaid of ${CurrencyFormatter.format(loan.principalAmount)}",
                        style = MaterialTheme.typography.bodyMedium,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                        modifier = Modifier.padding(top = 4.dp)
                    )
                    loan.dueDateMillis?.let {
                        Text(
                            text = "Due: ${SimpleDateFormat("d MMM yyyy", Locale.getDefault()).format(Date(it))}",
                            style = MaterialTheme.typography.bodyMedium,
                            modifier = Modifier.padding(top = 8.dp)
                        )
                    }
                    loan.interestRatePercent?.let {
                        Text(
            ...
            if (!loan.isSettled) {
                Card {
                    Column(modifier = Modifier.padding(16.dp)) {
                        Text(text = "Record a Repayment", style = MaterialTheme.typography.titleMedium)
                        OutlinedTextField(
